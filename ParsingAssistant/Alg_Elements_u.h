...
public:
	...
	void SetLeft(const ItemSymb& inp_l) { left = inp_l; }
	void SetLeftAsNonterminal() { left.SetAsNonterminal(); }
	void SetLeftAsAxiom() { left.SetAsAxiom(); }
	void AddOption(const ItemString& inp_op) { right.push_back(inp_op); }
...
struct RuleNum { 
	...
	bool IsFound() { return ((fir_num > -1) && (sec_num > -1)); }
...
public:
	...
	bool HasNoOffset() { return ((offset == RuleNum{0, 0}) && (entry_point_offset == -1)); }
...
public:
	...
	void MarkAsParsedEnd() { status = TypeOfTtoDLine::PARSED_END_TtoD; }
